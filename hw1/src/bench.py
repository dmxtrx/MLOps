...
def main() -> None:
    ...
    # Замер загрузки — отдельный таймер, генерация в него не попадает.
    t0 = time.perf_counter()
    tokenizer, model = load_model(params)
    load_time = time.perf_counter() - t0

    # Прогрев: аллокации, компиляция ядер, прогрев кэшей — не засчитывается.
    ...
    for _ in range(params["bench"]["measure_runs"]):
        ...
        speeds.append(n_tokens / (time.perf_counter() - t_run))
