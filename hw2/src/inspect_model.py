...
def parameter_rows(model) -> list[dict]:
    ...
    for name, param in model.named_parameters(remove_duplicate=False):
        # Тот же объект под вторым именем: у Qwen3 lm_head.weight — это
        # и есть embed_tokens.weight. Первое вхождение считаем, второе
        # помечаем как переиспользованное.
...
def forward_hooks(modules: dict) -> tuple[dict, list]:
    """Навесить forward-hooks и вернуть приёмник вместе с handle'ами.

    Handle возвращаются не для красоты: без них хук нечем снять, и каждый
    следующий прогон в том же процессе вешает ещё один комплект.
    """
...
def device_allocated_bytes(device: torch.device) -> int:
    """Сколько памяти занято прямо сейчас — метрикой под устройство.

    На mps и cuda тензоры живут в памяти ускорителя и в RSS процесса
    почти не попадают: мерить их через ru_maxrss — значит мерить не то.
    """
    if device.type == "cuda":
...
def device_metric_source(device: torch.device) -> str:
    """Имя функции, которой снята память, — слово в слово для отчёта."""
    if device.type == "cuda":
...
class PeakMemory:
    """Пик памяти за время блока, а не остаток после него.

    У mps счётчика high-water mark нет, поэтому пик набирается опросом:
    фоновый поток раз в interval секунд смотрит текущее потребление и
    держит максимум. На cuda max_memory_allocated сам по себе уже пик,
    и опрос ему не мешает.
    """
    ...
    def __exit__(self, *exc) -> bool:
        ...
        if self._thread is not None:
            self._thread.join()
        # Ещё один замер после остановки потока: пик мог прийтись
        # на последние миллисекунды блока и не попасть в опрос.
        self.used = max(self.used, device_allocated_bytes(self.device))
        gc.collect()
        return False
...
def probe_once(mode: str) -> dict:
    """Один замер в отдельном процессе.

    Пиковые метрики — high-water mark за жизнь процесса, и сбросить их
    нельзя. Померив три режима подряд в одном процессе, получишь три раза
    пик самого тяжёлого из них, а аллокатор Metal вдобавок держит
    освобождённые буферы в кеше.
    """
    proc = subprocess.run(
    ...
    if proc.returncode != 0:
        raise RuntimeError(
            f"замер режима {mode} упал с кодом {proc.returncode}:\n"
...
def memory_profile(params: dict) -> list[dict]:
    ...
    results = []
    for mode in MODES:
        runs = [probe_once(mode) for _ in range(repeats)]
